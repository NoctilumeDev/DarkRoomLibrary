...
| 自动测试 | 后端 165 项、前端 26 项；五角色全链路 75 次真实 API 响应，并发一致性 19 个场景 / 392 次请求 |
...
4. **可靠性闭环**：Redis、RabbitMQ 和邮件不是核心事务的单点依赖；不可用时回到内存、MySQL、同步写库或补偿任务。
...
| RabbitMQ 不可用 | 操作日志同步写库；通知任务进入补偿表 |
...
- 后端 `165/165` 通过，前端单元测试 `26/26` 通过。
- ESLint、Vite 生产构建和 npm 官方 registry 安全审计通过，审计结果为 `0 vulnerabilities`。
- 五个演示账号全部真实登录，完整流程记录 75 次 API 响应，覆盖借还、权限切换、采购、物流、入库和库存幂等。
- 并发一致性测试覆盖 19 个场景、392 次请求，最大 P95 为 99 ms，`429=0`、`500=0`。
- 浏览器诊断完成 86 个路由检查；Console 错误/警告、页面异常、失败请求、网络错误和页面横向溢出均为 0。
- 数据库完成 27 条外键关系检查，孤儿记录与领域不变量违规均为 0。
- Redis 使用真实缓存键和 TTL 验证；RabbitMQ 队列消费完成，`operation_log` 留下 72 条已消费记录。
...
这是计算机专业大二短学期课程作业的后续独立重构版本。课程阶段从教师提供的未完成教学脚手架开始，教师没有提供、也无法确认脚手架原始来源和许可证。完成课程提交后，作者在暑假继续补齐业务闭环、迁移 Vue 2 遗留结构到 Vue 3/Vite、升级后端到 Spring Boot 3、重构数据库一致性与权限边界，并补充自动测试、全链路验证和交付文档。
...
本项目起点是学校教师提供、但未附来源和许可证的教学脚手架。当前版本已经完成大规模独立重构，但在确认原脚手架授权之前，仓库不附加项目级开源许可证，也不宣称全部历史代码均为原创。

公开到 GitHub 只能视为“源码可见”，不能视为已经授予他人复制、修改或再分发权。具体来源判断、已完成的独立工作和后续授权条件见 [NOTICE.md](NOTICE.md)。
