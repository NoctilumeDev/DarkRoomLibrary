...
| 验证维度 | v1.2.0 最终结果 |
| --- | --- |
| 自动测试 | 后端 `241/241`，前端 `37/37`，ESLint、生产构建和 npm 审计通过 |
| 角色链路 | 5 个角色码、6 个固定权限身份全部真实登录并完成端到端流程 |
| 多实例一致性 | 3 个后端实例共享 MySQL；三批强并发基线共 1,986 次场景请求，业务不变量违规为 0 |
| 推荐并发 | 8 个首次并发请求只生成 1 个批次，三实例得到一致的协同结果 |
| 浏览器诊断 | 116 个路由、352 次 API 响应、6,102 次网络响应；Console、失败请求与横向溢出为 0 |
