...
本模块图对应 2026-07-27 公开基线与 v1.2.0 增量：Spring Boot 3.5.16、Vue 3.5.40、Vite 8.1.5，后端端口 `20606`、前端端口 `5175`。最终验证为后端 241 项、前端 37 项、6 个固定权限身份全链路通过；既有三个后端实例强并发基线为 1,986 次场景请求，新增推荐在三个实例同时首次生成时只产生一个批次，并以真实临时数据验证三实例 `HYBRID` 协同一致；最新浏览器诊断完成 116 个路由检查、352 次 API 响应和 6,102 次总网络响应，详情见 [`verification-report.md`](verification-report.md) 与 [`architecture-review.md`](architecture-review.md)。
