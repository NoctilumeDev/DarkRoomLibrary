...
同一读者生成推荐时先锁用户行，再计算输入指纹并查找 6 小时可复用批次。该锁属于共享 MySQL 事务边界，三个应用实例同时首次请求时也只会有一个实例生成，其余实例在取得锁后读取同一批次。生成时间按数据库秒精度写入，首次响应与复用响应不会因 JVM 纳秒精度产生不一致。
...
| 同一读者同时生成推荐 | 用户行锁 + 输入指纹 + 可复用批次 | 跨实例只生成一个批次 |
...
- 浏览器 / F12 等价诊断覆盖 `116` 个路由检查、`414` 次 API 响应和 `5,678` 次总网络响应；另检查 `116` 个页面和 `21` 个关键弹层。Console、页面异常、失败请求、网络错误和横向溢出均为 `0`。
- v1.2.0 推荐增量回归把 `8` 个首次生成请求分发到 `3` 个后端实例，只产生 `1` 个批次；最新浏览器诊断为 `116` 个路由、`352` 次 API 和 `6,060` 次网络响应，运行时错误仍为 `0`。
