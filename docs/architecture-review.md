...
- 后端 Maven 测试 `241/241` 通过。
- 前端单元测试 `37/37` 通过。
- 5 个角色码对应的 6 个固定权限身份完成 `78` 次真实 API 响应，采购入库使库存从 `6 -> 8`。
- 三个后端实例共享 MySQL，按突发量 `96`、`128`、`160` 分三批执行；每批 `8` 个一致性场景，三批共 `1,986` 次场景请求，最大场景 P95 为 `461 ms`。
- v1.0 历史完整布局基线覆盖 `116` 个路由检查、`414` 次 API 响应和 `5,678` 次总网络响应，并检查 `116` 个页面和 `21` 个关键弹层；该组数字只保留发布演进证据。
- v1.2.0 推荐增量回归把 `8` 个首次生成请求分发到 `3` 个后端实例，只产生 `1` 个批次，并以临时真实数据在三个实例上得到一致的 `HYBRID` 协同结果；最新浏览器诊断为 `116` 个路由、`352` 次 API 和 `6,102` 次网络响应，运行时错误仍为 `0`。
