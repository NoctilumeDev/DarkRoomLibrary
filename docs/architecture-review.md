...
本审查针对 2026-08-02 的 v1.2.1 冻结基线，覆盖：
...
2026-08-02 的 v1.2.1 冻结证据为：

- 后端 Maven 测试 `245/245`、前端单元测试 `41/41` 通过。
- 5 个角色码对应的 6 个固定权限身份完成 `70` 次真实 API 响应，采购入库使库存从 `6 -> 8`。
- 三个后端实例共享 MySQL，以突发量 `48` 完成 `8` 个一致性场景、`262` 次请求，三个 JVM 各承担三分之一的主要突发流量，最大场景 P95 为 `217 ms`。
- 另以 `76` 次真实请求完成 4 个边界场景：跨实例密码/角色变更使旧 JWT 失效、登录错误不泄露账号状态、采购员不能直接写入发货状态、消息只标记已展示记录。
- 浏览器诊断覆盖 `116` 个桌面与移动路由、`456` 次 API 响应和 `5,988` 次网络响应，Console、页面异常、失败请求、业务错误和横向溢出均为 `0`。
