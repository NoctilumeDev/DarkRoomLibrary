...
- 接入层先按来源 IP 做全路径限流，再按已认证用户或匿名 IP 做业务限流；连续登录失败按“账号 + 来源 IP”触发临时锁定，可信代理头必须显式开启。
...
2026-08-02 的 v1.2.1 冻结验收为：后端 245 项、前端 41 项全部通过；6 个固定权限身份完成 70 次真实 API 全链路；三个后端实例以 48 路突发完成 8 个一致性场景、262 次请求，另以 76 次请求验证 4 个认证与采购消息边界；浏览器诊断完成 116 个路由检查、456 次 API 响应和 5,988 次总网络响应。历史三批强并发基线 1,986 次场景请求与推荐三实例确定性结果继续保留；数据库库存范围、活动借阅/预约唯一性、推荐事件唯一性、采购入库幂等、邮箱配额和 34 个外键关系均无异常。完整证据见 [`verification-report.md`](verification-report.md)。
