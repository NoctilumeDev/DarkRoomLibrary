...
- GitHub Actions 对后端测试、前端静态检查/单测/构建/依赖审计和 Compose 配置执行持续验证；JaCoCo 与 Vitest V8 报告随 Pages 公开发布，后端和前端测试范围分别执行至少 70% 的行覆盖率门禁。
...
2026-08-02 的 v1.2.2 冻结验收为：后端 272 项、前端 58 项全部通过，两端行覆盖率门禁均达到 70%；6 个固定权限身份完成 73 次真实 API 全链路；三个后端实例完成 8 个一致性场景、176 次请求，独立并发套件完成 20 个场景、393 次请求，另以 76 次请求验证 4 个认证与采购消息边界；浏览器诊断完成 116 个路由检查、456 次 API 响应和 5,998 次总网络响应。共享 Redis 令牌桶、受信代理链、RabbitMQ 毒消息队列和通知终态 Webhook 已通过真实基础设施验证；历史三批强并发基线 1,986 次场景请求与推荐三实例确定性结果继续保留。数据库库存范围、活动借阅/预约唯一性、推荐事件唯一性、采购入库幂等、邮箱配额和 34 个外键关系均无异常。完整证据见 [`verification-report.md`](verification-report.md)。
