...
推荐输出保存在 `recommendation_batch` 与 `recommendation_item`，曝光、点击和推荐后收藏以不可变事件写入 `recommendation_event`；`recommendation_user_setting` 保存读者开关。生成时先锁定当前用户行，同一读者跨实例只能串行生成，输入指纹未变且 6 小时内直接复用批次。时间统一截断到数据库秒精度，保证首个生成响应与后续复用响应一致。
...
- 图表仅注册柱状图、折线图、饼图、网格、提示框和 Canvas 渲染器，并单独生成延迟分包，不进入登录页首屏。
...
- 推荐测试覆盖三收藏阈值、内容/公共/协同模式、过滤与多样性、确定性排序、隐私开关、事件归因和历史清除；三实例 E2E 要求同时首次生成只落一个批次。
...
2026-07-27 公开基线与 v1.2.0 增量回归为：后端 237 项、前端 37 项全部通过；6 个固定权限身份全链路通过；既有三个后端实例强并发基线按突发量 96、128、160 分三批执行 1,986 次场景请求，新增推荐再以 3 个实例同时首次生成，最终只落一个确定性批次；最新浏览器诊断完成 116 个路由检查、352 次 API 响应和 6,060 次总网络响应；数据库库存范围、活动借阅/预约唯一性、推荐事件唯一性、采购入库幂等、邮箱配额和 34 个外键关系均无异常。完整证据见 [`verification-report.md`](verification-report.md)。
