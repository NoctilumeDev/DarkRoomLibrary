...
| 敏感端点限流 | Redis Lua 令牌桶 | 本地令牌桶 | 否 |
| 查询缓存 | Redis | 直接查询 MySQL | 否 |
| 操作日志 | 业务提交后单独写 MySQL | 记录失败并保留错误日志 | 否；核心业务不因审计失败回滚 |
| 通知事件 | RabbitMQ 异步消费 | 写入 `notification_task` | 否 |
| 邮件发送 | SMTP | 记录失败并定时重试，终态可发 Webhook | 借还/预约不受影响 |
| 死信积压告警 | RabbitMQ 队列检查 + Redis 跨实例抑制 | 本实例冷却窗口 | 否 |
...
- JWT 携带 `auth_version`；密码、角色、协调员能力、冻结/解冻、禁用/启用或注销变化后，MySQL 认证版本递增，三个后端实例从下一次请求起共同拒绝旧令牌。
- 登录数学验证码、邮箱验证码用途隔离、重发间隔和每日发送尝试上限；邮件配置缺失或投递失败会释放短重发槽，但不会退还当日尝试预算。
- 同一规范化邮箱最多关联 3 个账号；界面公开固定规则，但接口不返回当前关联数量或账号身份。
- 公开注册先验证邮箱控制权，再检查账号、署名和邮箱配额；个人资料换绑使用受登录保护的 `CHANGE_EMAIL` 验证码。
- 接入层先按来源 IP 做全路径限流，再按已认证用户或匿名 IP 做业务限流；登录、注册、验证码和重置密码另有 Redis 令牌桶。连续登录失败按“账号 + 来源 IP”触发临时锁定。
- 默认忽略 `X-Forwarded-For` 与 `X-Real-IP`。只有直连地址命中受信代理 CIDR 白名单时才从右向左解析代理链；限流、登录保护和操作审计共用同一来源 IP。
- 每个响应返回并记录 `X-Request-ID`；只保留符合长度和字符白名单的调用方 ID，其余替换为服务端 UUID。
- 生产 Nginx 设置 CSP、禁止页面嵌入、限制 Referrer 和浏览器敏感能力，并保留 `nosniff`；静态入口使用 CSP Meta 作为托管兜底。
...
- GitHub Actions 对后端测试、前端静态检查/单测/构建/依赖审计和 Compose 配置执行持续验证；JaCoCo 对后端模块执行至少 70% 的行覆盖率门禁，Vitest V8 对 `src/demo/adapter.js` 与 `src/utils/**/*.js` 关键逻辑范围执行至少 70% 的行覆盖率门禁。Vue 组件和页面路由另由组件测试及 E2E 证明。
...
- 前端提供规则、路由守卫、登录和个人资料弹层组件测试，以及读者、管理员、采购物流、全流程、并发一致性和浏览器诊断脚本。
- 三实例边界 E2E 验证跨实例密码/角色变更使旧令牌失效、登录失败信息不泄露账号状态、采购员不能绕过物流伪造发货，以及消息只确认已展示记录。
- 自动回归覆盖 Redis/RabbitMQ 正常链路、核心降级逻辑、请求 ID、死信积压监控与生产配置拒绝；人工验收继续负责真实 SMTP 收件和部署环境差异。

截至 2026-08-28，当前公开版本 v1.2.7 的自动化基线为后端 `290/290`、前端 `68/68`；六身份 70 次页面 API、20 个并发场景 / 393 次请求以及浏览器 116 个路由 / 456 次 API / 6,114 次网络响应沿用 v1.2.4 的真实验收。三个后端实例的 8 个一致性场景 / 176 次请求保留为 v1.2.3 历史基线。

作为三实例边界的历史冻结证据，2026-08-02 的 v1.2.3 验收为：后端 282 项、前端 68 项全部通过，后端模块与前端关键逻辑范围的行覆盖率门禁均达到 70%；6 个固定权限身份完成 73 次真实 API 全链路；三个后端实例完成 8 个一致性场景、176 次请求，独立并发套件完成 20 个场景、393 次请求，另以 76 次请求验证 4 个认证与采购消息边界；浏览器诊断完成 116 个路由检查、456 次 API 响应和 6,206 次总网络响应。共享 Redis 令牌桶、受信代理链、请求 ID、CSP、安全响应头、RabbitMQ 毒消息队列、通知终态 Webhook 和多实例死信积压告警已通过真实基础设施验证；历史三批强并发基线 1,986 次场景请求与推荐三实例确定性结果继续保留。数据库库存范围、活动借阅/预约唯一性、推荐事件唯一性、采购入库幂等、邮箱配额和 34 个外键关系均无异常。当前机器可读口径与完整证据见 [验证基线](../.github/verification-baseline.json) 和 [`verification-report.md`](verification-report.md)。
