...
2026-07-27 最终自动化基线为：后端 226 项、前端 36 项全部通过；6 个固定权限身份完成 78 次真实 API 响应；三个后端实例共享 MySQL，按突发量 96、128、160 分三批执行，每批 8 个一致性场景，三批共 1,986 次场景请求，最大场景 P95 为 461 ms；浏览器完成 116 个路由检查、414 次 API 响应和 5,678 次总网络响应，并完成 116 个页面与 21 个关键弹层的布局审查；数据库库存范围、活动借阅/预约唯一性、采购入库幂等和 27 个外键关系均无异常。完整证据见 [`verification-report.md`](verification-report.md)。
