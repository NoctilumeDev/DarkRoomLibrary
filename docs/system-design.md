...
本文解释系统为什么这样分层、五类角色怎样协作、核心数据如何保持一致，以及 Redis、RabbitMQ 和邮件不可用时系统如何继续工作。
...
| 操作日志 | RabbitMQ 异步写库 | 同步写 MySQL | 否 |
...
2026-07-26 最终自动化基线为：后端 165 项、前端 26 项全部通过；五个角色完成 75 次真实 API 响应；并发一致性覆盖 19 个场景、392 次请求，最大 P95 为 99 ms；浏览器完成 86 个路由检查；数据库库存范围、活动借阅/预约唯一性、采购入库幂等和 27 个外键关系均无异常。完整证据见 [`verification-report.md`](verification-report.md)。
